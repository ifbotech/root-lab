...
describe('reconocer la planta', () => {
  ...
  test('con el primer reconocimiento nace la ficha y el prompt del chat', async () => {
    const esc = escenario();
    const { token, planta } = await conRooti(esc, { persona: 'nori', nombre: 'Lola' });
    ...
    assert.match(guardada.prompt, /Rooti llamado Nori/);
    assert.match(guardada.prompt, /crítica y sofisticada/);
...
describe('charlar con la planta', () => {
  async function plantaLista(esc, opciones = {}) {
    const r = await conRooti(esc, { persona: 'kip', nombre: 'Rulo', ...opciones });
